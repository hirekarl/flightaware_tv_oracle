...
- [x] Rate limiting on `/api/fleet/stream` (concurrent connection cap + 429 + Retry-After)
- [x] Integration tests for SSE endpoint (HTTP contract, event format, payload schema, rate limit)
- [x] Parallelize coordinator LLM calls in `_sse_fleet_stream` with `asyncio.gather()`
...
- [x] Dependabot config for automated dependency PRs (pip, npm, Actions)
